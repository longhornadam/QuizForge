"""Parser for the legacy plain-text outline format used by the LLM workflow."""
...
from .base import QuizParser


class TextOutlineParser:
    """Concrete parser for the TXT authoring format produced by the LLM."""

    def parse(self, path: str, *, title_override: Optional[str] = None) -> Quiz:
        """Parse a quiz from a file path.

        Args:
            path: File path to a .txt quiz outline
        ...
        with open(path, "r", encoding="utf-8") as handle:
            content = handle.read()
        return self.parse_text(content, title_override=title_override, filename=os.path.basename(path))

    def parse_text(self, content: str, *, title_override: Optional[str] = None, filename: str = "quiz") -> Quiz:
        ...
        if not title:
            title = os.path.splitext(filename)[0] if filename else "Untitled Quiz"
        if title_override:

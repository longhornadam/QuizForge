...
This module provides post-generation inspection of QTI packages:
- summarize_quiz(): Human-readable quiz summary
- inspect_qti_package(): Parse ZIP, verify structure, count question types

Validation ensures the generated package is structurally sound before Canvas import.

Key class:
- PackageInspection: Dataclass with assessment path, item count, question type counts
"""
...
"""Summary of a packaged QTI assessment: path, item count, question type breakdown."""
...
def summarize_quiz(quiz: Quiz) -> str:
    """Return human-readable quiz summary (title, item count, question types and points)."""
...
def assert_questions_present(questions: Iterable[Question]) -> None:
    """Raise ValueError if question list is empty."""
...
def inspect_qti_package(zip_bytes: bytes) -> PackageInspection:
    """Parse ZIP archive and verify structural integrity (manifest, assessment, items). Return PackageInspection."""

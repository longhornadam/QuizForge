...
def add_passage_numbering(text: str, passage_type: str = "auto") -> Tuple[str, str]:
    ...
    if passage_type == "auto":
        ...
        if avg_line_len < 60 and len(non_empty) > 3:
            passage_type = "poetry"
        elif paragraph_breaks < 3:
            passage_type = "prose_short"
        else:
            passage_type = "prose_long"
    ...
    if passage_type == "prose_short":
        numbered: list[str] = []
        line_num = 1
        for line in lines:
            if line.strip():
                numbered.append(f"{{LINENUM:{line_num:2}}}     {line}")
                line_num += 1
            else:
                numbered.append("")
        return "\n".join(numbered), "prose_short"

    if passage_type == "prose_long":
...
def htmlize_prompt(text: str, *, excerpt_numbering: bool = True) -> str:
    ...
    def render_excerpt_block(content: str, forced_type: Optional[str] = None) -> str:
        ...
        if passage_type in {"prose_short", "prose_long"}:

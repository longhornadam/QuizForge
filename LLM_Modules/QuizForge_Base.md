...
- **FITB** — `prompt` must contain one or more bracketed blank ids (e.g., `[blank]` for a single blank, `[blank1]`, `[blank2]` for multiple). `accept` is a list per blank, each an array of acceptable answers in the order blanks appear. `case_sensitive` defaults to `false`; leave it `false` or include case variants unless the teacher explicitly requires case-sensitive grading (Canvas open-entry matching is otherwise case-sensitive). Optional `answer_mode`: `"open_entry"` (default), `"dropdown"`, or `"wordbank"`; `dropdown`/`wordbank` require an `options` array that includes the correct answer(s).
- **FITB modes & multi-blank** — Dropdown/wordbank are currently single-blank only; multi-blank FITB is open-entry and splits scoring equally across blanks. Provide `[blank1]`, `[blank2]`, etc., with matching `accept` arrays in order.
...
- Counts/answers: MC/MA have 2-7 choices; MC exactly 1 correct; MA at least 1 correct; TF is boolean; FITB has one or more `[blank_id]` tokens + `accept`; ORDERING has >=2 items; CATEGORIZATION has prompt + categories (>=2) + labeled items; NUMERICAL has `answer` (+ `evaluation` if not exact).
